...
import { PROPERTIES, Property, PropertyType } from '../lib/properties'
...
export default function BiensPage() {
  ...
  return (
    <main className="bg-white text-slate-900 min-h-screen">
      ...
      {/* FLOATING WIDGET — auto-open when triggered */}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-8" onClick={onClose}>
      ...
      >
        ...
        <div className="relative flex-shrink-0" style={{ height: 260 }}>
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-9 h-9 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60 transition-colors text-lg"
          >
            ✕
          </button>
          <span className={`absolute top-4 left-4 text-xs font-bold text-white px-3 py-1 rounded-full ${p.badgeColor}`}>
            {p.badge}
          </span>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {p.features.map(f => (
                <span key={f} className="text-xs font-medium px-3 py-1.5 rounded-full bg-amber-50 text-amber-700 border border-amber-100">
                  ✓ {f}
                </span>
              ))}
            </div>
