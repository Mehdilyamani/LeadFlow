      ...
      <section className="bg-[#0c2033] pt-[72px] text-white sm:pt-[82px]">
        <div className="mx-auto grid min-h-[720px] max-w-[1600px] lg:grid-cols-[.92fr_1.08fr]">
          <div className="flex items-center px-4 py-16 sm:px-8 sm:py-20 lg:px-12 xl:px-20">
            <div className="demo-hero-enter w-full min-w-0 max-w-[650px]">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link href={getBrandHref(brand, '/biens')} className="inline-flex min-h-12 items-center justify-center gap-2 bg-[#c69a62] px-6 text-[11px] font-semibold text-[#0c2033] transition-colors hover:bg-[#dfb77f]">
          ...
          <div className="relative min-h-[440px] overflow-hidden lg:min-h-full">
            <Image src="/demos/immo-built/hero-cfc.webp" alt="Architecture contemporaine à Casablanca" fill priority sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover" />
      ...
      <section className="px-4 py-20 sm:px-7 sm:py-28 lg:px-12">
        <div className="mx-auto max-w-[1440px]">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            ...
            <ImmoReveal delay={0.08}><Link href={getBrandHref(brand, '/biens')} className="inline-flex items-center gap-2 border-b border-[#9b7446] pb-1.5 text-[11px] font-semibold text-[#765631]">Voir tous les biens <ArrowRight className="h-3.5 w-3.5" /></Link></ImmoReveal>
          </div>
          <div className="mt-12 grid gap-x-5 gap-y-9 md:grid-cols-2 lg:grid-cols-3">
            {IMMO_BUILT_PROPERTIES.map((property, index) => <ImmoReveal key={property.id} delay={Math.min(index * 0.04, 0.12)}><ImmoPropertyCard brand={brand} property={property} /></ImmoReveal>)}
          </div>
        </div>
      </section>

      <section className="demo-deferred-section bg-white px-4 py-20 sm:px-7 sm:py-24 lg:px-12">
        <div className="mx-auto max-w-[1440px]">
